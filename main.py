...
async def text(message) -> None:
    ...
    if message.text == '⁉️Информация':
        panel = types.ReplyKeyboardMarkup(resize_keyboard=True)
        await bot.send_message(message.chat.id,
                               '''📌Раздел информации об оплате покупки.\nBoxPay - бот через который происходит оплата любой покупки. Есть два типа оплаты:\n   1. 💳Оплатить покупку (online) - это оплата через онлайн платежную систему, платеж занимает всего навсего 1 секунду ( быстро и надежно ).
                         \n   2. 💵Оплатить покупку (через представителя) - если вы боитесь оплачивать через бота ( или нам не доверяете ), тогда эта оплата для вас, но она занимает времени от 24 часов до 48 часов. Вам будет отправлена ссылка на нашего представителя и он поможет вам с оплатой!
                         \n\n Когда я смогу увидеть свою покупку?\nСразу после оплаты мы отправим товар на указанный вами адрес!
                         \n\n Я могу оплатить курс где угодно? Из любой точки мира?\n Да, совершенно верно! Вы можете оплатите покупку из любой точки мира!''',
                               reply_markup=panel)
