...
def auth_button():
    ...
    def log_in():
        def update_clock():
            ...
            end   = datetime.datetime(2021,5,10,int(time.strftime("%H")),int(time.strftime("%M")),int(time.strftime("%S")))
            delta = end-start
                    ...
                    if result.getbbox() == None:
                        if CURRENT_VOICE_FILE_NAME == '':
                            if str.lower(string[5][:-1]) != CURRENT_VOICE_FILE_NAME:
                                ...
                                inic = Label(wn5, text="Добро пожаловать, "+current_name+' '+current_last_name ,font=("Times New Roman", 18))
                                ...
                                try:
                                    ...
                                    for line in f2:
                                        k=k+1
                                        var1[k]=Label(wn5, text=line,font=("Times New Roman", 14)).place(x=50,y=60+k*50)
                                    k=0
                                except:
                                    Label(wn5, text="Актуальные задания отсутствуют!",font=("Times New Roman", 14)).place(x=50,y=120)
                                ...
                                try:
                                    ...
                                    f1.close()
                                    clrnr = Button(wn5,text="Очистить", font="15",command = clear_notice).place(x='700',y='450', height =30, width=130)
    ...
    def mainpanel():
        def jornal():
            ...
            try:
                ...
                Button(wn6,text="Очистить журнал",font="15",command=clear_journal).place(x='500',y='520', height =25, width=160)
            except:
