#Код для этой лабораторной работы юыл взят из репозитория:
#https://github.com/sobolevn/python-code-disasters/blob/master/python/player.py
#!usr/bin/env python
"""
#######################################################
# Python Music Player
...
def LogErr():
    try:
        ...
        if debug:
            bcast("[Error]: " 
                  + ''.join(line for line in lines), True)
    except:
        pass
    ...
    if len(sys.argv) > 1:
        ...
        while i < len(sys.argv):
            ...
            if arg == '--console' or arg == '-c':
                ...
                print ("\t -c, --console\t \
                    Disables Pygame screen (text-only mode)")
                print ("\t -f, --file\t \
                    Plays the file at the filepath specified")
                print ("\nExamples: \n\t " 
                       + sys.argv[0] 
